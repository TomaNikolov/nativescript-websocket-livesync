...
function FilePackage(file, pubSub, receiving) {
  ...
  if (receiving) {
    ...
    var sourceFile = fs.File.fromPath(file.path);
    this.buffer = sourceFile.readSync();
    if (this.isText) {
...
FilePackage.prototype.getMimeType = function () {
  this.mimeType = mimetype(`.${this.path.split('.').pop()}`);
};
...
FilePackage.prototype.makeBuffer = function () {
  this.buffer = helper.makeBuffer(this.data);
